...
- **플래그**: `--auto`(또는 `자동으로`), `--continuous`(또는 `연속으로`), `--skip-analysis`, `--skip-design`, `--skip-plan`, `--skip-verify` (선두에 위치)
- **업무 지시**: 플래그를 제외한 나머지 텍스트 (빈 경우 로드맵에서 다음 항목 자동 선택)
...
- `/task-start --continuous` → 연속 모드, 로드맵에서 다음 미착수 항목부터 자동 진행
- `/task-start --continuous --auto` → 연속+자동 모드, 체크포인트 없이 로드맵 항목을 연속 진행
...
| 연속 | `--continuous`, `연속으로` | 작업 완료 후 팀 정리 → 같은 지시를 새 팀으로 재실행 (반복) |

### 연속 모드 상세

연속 모드(`--continuous`)는 하나의 작업이 완료되면 팀을 정리(컨텍스트 해제)하고, **처음 받은 동일한 지시를 새 팀으로 다시 실행**하는 루프 모드다.

핵심 원리: 같은 지시라도 매번 **코드/로드맵 상태가 변했으므로 다른 작업을 수행**한다. 예를 들어 "로드맵 다음 항목 진행해줘"라는 지시를 반복하면, 새 PL이 매번 로드맵을 읽고 이전에 완료된 항목 다음 것을 수행한다.

**연속 모드 루프**:

```
초기 지시: $ARGUMENTS (플래그 제외한 업무 지시)

[루프 시작] — N번째 실행
  │
  ├─ 1. 새 팀 생성 + 일반 실행 절차 (1~4단계) 진행
  │     (PL은 새 컨텍스트에서 현재 코드/로드맵 상태를 기준으로 판단)
  │
  ├─ 2. project-leader가 완료 보고
  │     ├─ 커밋 필요 시 → 사용자에게 커밋 승인 요청 후 진행
  │     └─ 커밋 불필요 시 → 바로 다음 단계
  │
  ├─ 3. 팀 정리: TeamDelete(team_name="smart-fire-hub")
  │     → 기존 팀의 컨텍스트가 모두 해제됨
  │
  ├─ 4. 사용자에게 완료 보고:
  │     "### [N번째 완료] — {완료 요약}
  │      다음 실행을 시작합니다. (중단: '멈춰')"
  │
  ├─ 5. 같은 지시($ARGUMENTS)로 [루프 시작]으로 돌아감
  │
[루프 종료]
```
...
```bash
# 로드맵 항목을 하나씩 연속 자동 진행
/task-start --continuous 로드맵 다음 항목 진행해줘
# → 1회차: PL이 로드맵 읽고 Phase 4-1 수행 → 완료 → 팀 정리
# → 2회차: 새 PL이 로드맵 읽고 Phase 4-2 수행 (4-1은 이미 ✅) → 완료 → 팀 정리
# → 3회차: ...

# 자동 모드와 결합 — 체크포인트 없이 연속 진행
/task-start --continuous --auto 로드맵 다음 항목 진행해줘
```

**중단 조건** (루프를 멈추는 경우):
- 사용자가 "멈춰", "중단", "stop" 지시
- PL이 "더 이상 수행할 작업 없음"으로 완료 보고 (예: 로드맵 항목 전부 ✅)
- 블로커 발생으로 사용자 판단 필요
- 검증 실패 3회 연속 반복
...
```
# 1. 기존 팀 삭제 (컨텍스트 해제)
TeamDelete(team_name="smart-fire-hub")

# 2. 사용자에게 상태 표시
"### [N번째 완료] — {완료 요약}
> 같은 지시로 다음 실행을 시작합니다..."

# 3. 같은 지시로 새 팀 생성 (1단계부터 다시 시작)
TeamCreate(team_name="smart-fire-hub")
Agent(name="project-leader", prompt="업무 지시: {원래 $ARGUMENTS 그대로}")
Agent(name="analyst", ...)
```

**연속 모드에서 유지되는 것**:
- 원래 업무 지시 ($ARGUMENTS) — 매 루프에서 동일하게 전달
- 실행 모드 플래그 (`--auto`, `--skip-*` 등)
- 메인 Claude의 컨텍스트 (사용자와의 대화 이력, 루프 카운트)

**연속 모드에서 초기화되는 것**:
- 팀 멤버 전체 (TeamDelete로 해제)
- 팀원들의 작업 컨텍스트 (이전 작업의 코드 분석, 설계 등)
- TaskList (새 팀에서 새로 생성)

**커밋 처리**: 연속 모드에서도 커밋은 반드시 사용자 승인 후 진행한다.
