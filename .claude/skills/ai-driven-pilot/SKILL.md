...
# 1. ai-fix 라벨 없음 → 사람 큐 (스킵) — 옵트인 방식: 사람이 검토 후 ai-fix 붙여야 파일럿 처리
# 2. needs-info / pilot:escalated / on-hold / deferred / security 라벨 → 사람 큐 (스킵)
# 3. regression 라벨 + ai-fix 있음 → solver 재처리 큐
# 4. resolved 라벨 + ai-fix 있음 → explorer 크로스체크 큐
# 5. ai-fix + severity:* + 위 어디에도 안 속함 → 신규 솔버 큐
```
...
| **Security** | 라벨 `security` 부착됨 | 코멘트 "보안 이슈는 사람 결정 후 close" + skip |
...
| `needs_human` | `ai-fix` 라벨 제거 + 이슈 코멘트(이유) + 일시정지 X, **다음 루프 계속** | `backlog` (사람 결정 후 ai-fix 재부착) |
...
| `test_failed_2` / `hook_failed_2` / `reproduce_failed` / `blocked` (위 외) | `pilot:escalated` 라벨 부착 + 코멘트 + **사이클 일시정지** (환경/분석 문제는 다음 이슈에서도 반복될 가능성) | `in_review` (사람 리뷰 필요) |
| RESULT 라인 자체가 없거나 형식 깨짐 | `blocked` 처리 + `pilot:processing` 제거 + 일시정지 (silent failure 방지) | `in_review` (사람 리뷰 필요) |
...
| solver RESULT: needs_human | `backlog` | 사람 결정 필요 (ai-fix 제거) |
| 에스컬레이션 / blocked | `in_review` | 사람 리뷰 필요 (ai-fix 라벨 검토 후 재부착) |
