...
# 1. ai-fix 라벨 없음 → 사람 큐 (스킵) — 단일 게이트. 사람이 검토 후 ai-fix 붙여야 파일럿 처리
#    (security / deferred / on-hold / needs-info 등 다른 라벨은 분류에 영향 없음.
#     처리 제외하려면 사람이 ai-fix를 떼면 됨.)
# 2. regression 라벨 + ai-fix 있음 → solver 재처리 큐
# 3. resolved 라벨 + ai-fix 있음 → explorer 크로스체크 큐
...
- 처리 제외 게이트는 **ai-fix 라벨 단일 채널**. 사람 개입이 필요하다고 판단되면 (난이도/처리범위/brainstorming·plan 필요 등) pilot이 ai-fix를 자동 제거하고 보드 `backlog`로 이동. 사람이 검토 후 ai-fix 재부착 시 자동 재처리. `pilot:escalated` 라벨은 사용하지 않는다.
