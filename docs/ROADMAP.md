...
| [Phase 10](#phase-10-channel-추상화--알림-인프라) | **완료** | 4/4 | Outbox 인프라 + KAKAO/SLACK outbound + Slack inbound AI + firehub-channel 마이크로서비스 분리 |
...
| 10-4 | firehub-channel 마이크로서비스 분리 | ✅ | Backend + Infra | Node.js/Express 얇은 HTTP 어댑터(port 3002). POST /send(SLACK/KAKAO/EMAIL), POST /slack/events(HMAC+3초ack+포워딩). Internal Auth 미들웨어. firehub-api: SlackInboundController(Internal 전용), ChannelHttpClient(WebClient), SlackChannel/KakaoChannel/EmailChannel→ChannelHttpClient 교체. SlackEventsController/SlackSignatureVerifier/SlackInboundAsyncConfig 제거. docker-compose+nginx 반영. 운영 배포 완료. |
...
| 날짜 | 변경 내용 |
|------|---------|
| 2026-04-19 | Phase 10-4 firehub-channel 마이크로서비스 분리 완료. Node.js/TypeScript Express 서비스(port 3002). POST /send(SLACK/KAKAO/EMAIL 외부 API 호출), POST /slack/events(HMAC-SHA256 서명검증+3초ack+비동기 포워딩). Internal Auth 미들웨어. firehub-api: SlackInboundController(POST /api/v1/channels/slack/inbound Internal 전용), ChannelHttpClient(WebClient POST /send 래퍼). SlackChannel/KakaoChannel/EmailChannel → ChannelHttpClient.send() 교체. SlackEventsController/SlackSignatureVerifier/SlackInboundAsyncConfig 제거. deploy.sh channel 케이스 추가. docker-compose firehub-channel 서비스 추가. nginx /slack/events → firehub-channel:3002 라우팅. 운영 배포 및 health check 완료. 스펙: docs/superpowers/specs/2026-04-19-firehub-channel-design.md, 플랜: docs/superpowers/plans/2026-04-19-firehub-channel-plan.md. |
| 2026-04-19 | Channel 추상화 Stage 3 (Slack Inbound — 양방향 AI) 완료. V55(ai_session에 channel_source/slack_team_id/slack_channel_id/slack_thread_ts 컬럼 추가 + uk_ai_session_slack_thread PARTIAL UNIQUE INDEX). SlackSignatureVerifier(HMAC-SHA256 v0= + ±5분 타임스탬프 + Signing Secret 교체 그레이스). SlackEventsController(POST /api/v1/channels/slack/events: url_verification challenge 응답 + event_callback 3초 ack + @Async dispatch). SlackInboundAsyncConfig(slackInboundExecutor core=3/max=5/queue=20/CallerRunsPolicy). AiAgentBatchClient(non-SSE WebClient: createSession 10s/chat 60s blocking). SlackApiClient 확장(reactionsAdd/:eyes:/:warning:, postEphemeral, chatPostMessageInThread). SlackChannel.replyTo(workspaceId, channel, threadTs, text). SlackInboundService(@Async 비동기: 워크스페이스 조회→봇토큰 복호화→reactions.add→user 매핑→ai_session lookup/create→AI chat→replyTo; 미연동 사용자 ephemeral 안내, AI 오류 시 warning reaction + ephemeral). UserChannelBindingRepository.findByExternalId(teamId, externalUserId) jOOQ JOIN. AiSessionRepository 확장(findBySlackContext/createSlackSession). SlackInboundMetrics(Micrometer: received_total, processing_duration_seconds, unmapped_user_total). MDC correlationId(slack-{teamId}-{ts}). SecurityConfig: /api/v1/channels/slack/events permitAll. 단위 테스트 9+5+4건 PASS(SlackSignatureVerifier/SlackInboundService/AiSessionRepositorySlack). 런북 섹션 9(Event Subscriptions 설정 + 환경변수 + 동작확인 + 풀튜닝 + 장애조치). 스펙: docs/superpowers/specs/2026-04-18-channel-abstraction-design.md, 플랜: docs/superpowers/plans/2026-04-19-channel-stage-3-slack-inbound.md. |
| 2026-04-19 | Channel 추상화 Stage 2 (KAKAO/SLACK outbound + 사용자 연동 UX) 완료. OAuthStateService(CSRF CSPRNG 32B hex, TTL 10m single-use). Kakao: KakaoApiClient(exchangeCode/refresh/sendMemoText), KakaoOAuthService(code→token upsert + AES-GCM 암호화), KakaoChannel(나에게 보내기 + refresh 1회 재시도 + 990자 트렁케이션 + 안내문구), KakaoTextFormatter. Slack: SlackApiClient(oauth.v2.access/chat.postMessage/users.info/conversations.open), SlackOAuthService(워크스페이스 설치 + linkUser DM ping), SlackChannel(binding→workspace bot_token→DM), SlackBlockKitRenderer(Jackson JSON escape 안전), SlackOAuthController(/api/v1/oauth/slack/{start,callback,link-user}). UserChannelBindingRepository 확장(upsert ON CONFLICT uk_user_channel + findByUser + revoke), UserChannelPreferenceRepository.setEnabled(CHAT IllegalArgument), SlackWorkspaceRepository(findById/upsertFromOAuth/revoke). 백엔드: ChannelSettingsController(GET/PATCH/DELETE /api/v1/channels/settings) + ChannelSettingsService(4채널 연동 상태+oauthStartUrl+displayAddress 조합) + DTO + 6 TC. 프론트엔드: `/settings/channels` 페이지(ChannelCard/ChannelStatusBadge/OAuthConnectButton 새창 + 팝업 닫힘 감지) + TanStack Query hooks + 7 E2E. ChannelRecipientEditor CHAT/EMAIL→CHAT/EMAIL/KAKAO/SLACK 4채널 확장 + Zod 스키마 확장 + 6 E2E. 런북 섹션 8(Kakao/Slack 앱 등록 + 환경변수 + dev→stage→운영 단계 + 장애 회귀). Kakao/Slack 단위 테스트 19+건 PASS, 프론트 E2E 13건 PASS. 스펙: docs/superpowers/specs/2026-04-18-channel-abstraction-design.md, 플랜: docs/superpowers/plans/2026-04-18-channel-stage-2-external-outbound.md. 실제 OAuth 자격 증명으로 end-to-end 검증은 운영 진입 단계에서 별도 수행. |
| 2026-04-18 | Channel 추상화 Stage 1 (Outbox 인프라) 완료. V50(notification_outbox) + V51~V54(slack_workspace/user_channel_binding/user_channel_preference/oauth_state). Channel/BoundChannel SPI + ChannelType 고정 AuthStrategy. RoutingResolver(opt-out/BINDING_MISSING 매트릭스 + CHAT 안전망) + BackoffPolicy(10s/1m/5m/30m/2h 5회). NotificationDispatcher(AFTER_COMMIT + idempotency + LISTEN/NOTIFY) + Worker(SKIP LOCKED + lease) + OutboxSweeper(5분 좀비 회복). ChatChannel(proactive_message + SSE) + EmailChannel(SMTP) 구현. ProactiveJobService 듀얼 패스 전환(feature flag `notification.outbox.enabled` 기본 OFF → 회귀 안전). 단위·통합 테스트 30+건 PASS. 스펙: docs/superpowers/specs/2026-04-18-channel-abstraction-design.md, 플랜: docs/superpowers/plans/2026-04-18-channel-stage-1-outbox.md, 런북: docs/runbooks/notification-outbox-rollout.md. Stage 2(KAKAO/SLACK outbound) + Stage 3(Slack inbound)는 후속 plan으로 분리. |
...
| 2026-04-02 | Phase 7-3 (리포트 내러티브 강화) 완료. 시스템 프롬프트 강화 (분석 원칙 + 섹션 타입별 가이드). 이전 실행 결과 3건 컨텍스트 전달. 템플릿 style 필드 (DB 컬럼 + DTO + 프론트엔드 입력란). Layer 1 완료. |
| 2026-04-02 | Phase 7-1 (PDF 리포트 내보내기) 완료. Flying Saucer HTML→PDF. ReportRenderUtils 공통 추출. PDF 다운로드 API + 버튼. 이메일 PDF 첨부(attachPdf). NanumGothic 한글 폰트. 7-2 Slack/Webhook 제거. |
| 2026-04-02 | Phase 7 Layer 0 전체 완료 (7-0a, 7-0b, 7-0c). 7-0a: 채널별 수신자 지정 + 스마트 작업 상세 페이지. 7-0b: 작업 복제/Cron 프리셋/타임존/에러 메시지 개선. |
| 2026-04-02 | Phase 7-0c (리포트 템플릿 UX 개선) 완료. 템플릿 상세 페이지 도입 (읽기/편집 모드 전환), CodeMirror JSON 에디터 (구문 강조 + 실시간 검증 + 삽입 툴바), 9가지 섹션 타입 가이드, 사이드패널 (가이드/미리보기), 빌트인 복제, 백엔드 단건 조회 API 추가. |
