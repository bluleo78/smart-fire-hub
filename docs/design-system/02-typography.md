...
|---------------|------|-----------------|------|-------------|--------|----------|
| `heading-page` | 페이지 타이틀 (H1) | `text-[28px] leading-[36px] font-semibold tracking-tight` | 28px | 36px | 600 | -0.025em |
...
`tracking-tight`(`letter-spacing: -0.025em`)은 **Heading 계층에서 28px 이상**에만 적용한다.

| 조건 | Tracking | 이유 |
|------|----------|------|
| `heading-page` (28px), `heading-section` (24px) | `tracking-tight` | 큰 글자는 자간이 넓어 보여 타이트하게 보정 |
...
- `text-[28px]`과 같은 arbitrary value는 그대로 지원.
- `leading-[36px]`도 arbitrary value로 지원.
