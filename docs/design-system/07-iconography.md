...
아이콘과 텍스트가 함께 있을 때 `gap-*`으로 간격을 제어한다. `margin`이나 `padding`을 아이콘에 직접 적용하지 않는다.

> **이 절이 아이콘 간격의 정본이다 (#436).** 08·09·05 의 예제에 남아 있던 `mr-2` 처방은
> 이 규칙으로 통일했다. 이유는 취향이 아니라 실제 시각 결함이다 — shadcn `Button` 은 기저
> 클래스에 이미 `gap-2`(`size="sm"` 은 `gap-1.5`)를 갖고 있어서, 버튼 안 아이콘에 `mr-2` 를
> 덧붙이면 간격이 8px 이 아니라 **16px** 로 벌어진다. 같은 화면에서 `mr-2` 가 붙은 버튼과
> 안 붙은 버튼의 간격이 두 배 차이 난다.
>
> **컨테이너에 `gap-*` 이 없다면** margin 을 지우기 전에 컨테이너에 `gap-2` 를 먼저 넣어야
> 한다. 그냥 지우면 간격이 0 이 된다.
