...
class SsrfProtectionTest {
    ...
    /**
     * Simulates the SSRF-via-redirect attack scenario:
     * an attacker's server issues a 301 redirect to http://169.254.169.254/ (cloud metadata endpoint).
     *
     * ApiCallExecutor.executeWithRedirects() calls ssrfProtectionService.validateUrl() on every
     ...
     *
     * The link-local range 169.254.0.0/16 is used by AWS/GCP/Azure metadata services and is
    ...
    void validateUrl_redirectToLinkLocal_blocked() throws UnknownHostException {
        // Direct IP form avoids external DNS — validates the protection mechanism without network calls.
    ...
    /**
     * Verifies that a redirect to a private RFC-1918 address is also blocked.
     * Covers the scenario where a redirect targets an internal service (e.g. http://10.0.0.1/).
     */
    @Test
    ...
    /**
     * Verifies that a redirect to a loopback address is blocked.
     * Covers the scenario where a redirect targets http://127.0.0.1/ to reach localhost services.
     */
    @Test
    ...
    /**
     * Verifies that validateUrl() rejects a URL whose hostname resolves to a link-local address.
     * This is the full-stack check: URL string -> hostname parse -> DNS resolve -> IP block.
     * Uses a literal IP URL to avoid DNS flakiness in CI.
     */
    @Test
    void validateUrl_linkLocalIpUrl_blocked() {
        ...
        assertThrows(SsrfException.class, () -> service.validateUrl("http://169.254.169.254/latest/meta-data/"));
    }
}
