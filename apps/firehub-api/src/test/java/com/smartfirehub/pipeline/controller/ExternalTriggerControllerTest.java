...
class ExternalTriggerControllerTest extends IntegrationTestBase {
    ...
    void setUp() {
        ...
        PipelineDetailResponse pipeline = pipelineService.createPipeline(
                new CreatePipelineRequest("Ext Pipeline " + unique, "Description", List.of()), testUserId);
    ...
    void apiTrigger_validToken_triggersExecution() throws Exception {
        TriggerResponse trigger = triggerService.createTrigger(pipelineId, new CreateTriggerRequest(
                "API Test", TriggerType.API, null, Map.of()), testUserId);
        String rawToken = (String) trigger.config().get("rawToken");

        mockMvc.perform(post("/api/v1/triggers/api/" + rawToken)
    ...
    void apiTrigger_invalidToken_returnsUnauthorized() throws Exception {
        mockMvc.perform(post("/api/v1/triggers/api/invalid-token-value")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{}"))
    ...
    void webhookTrigger_validWebhookId_triggersExecution() throws Exception {
        TriggerResponse trigger = triggerService.createTrigger(pipelineId, new CreateTriggerRequest(
                "Webhook Test", TriggerType.WEBHOOK, null, Map.of()), testUserId);
        String webhookId = (String) trigger.config().get("webhookId");

        mockMvc.perform(post("/api/v1/triggers/webhook/" + webhookId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"data\": \"test\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("triggered"));
    ...
    void webhookTrigger_invalidWebhookId_returnsNotFound() throws Exception {
        mockMvc.perform(post("/api/v1/triggers/webhook/non-existent-webhook-id")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{}"))
    ...
    void webhookTrigger_withSecret_missingSignature_returnsUnauthorized() throws Exception {
        TriggerResponse trigger = triggerService.createTrigger(pipelineId, new CreateTriggerRequest(
                "Secure Webhook", TriggerType.WEBHOOK, null, Map.of("secret", "my-secret-key")), testUserId);
        String webhookId = (String) trigger.config().get("webhookId");
        ...
        mockMvc.perform(post("/api/v1/triggers/webhook/" + webhookId)
                        ...
                        .content("{\"data\": \"test\"}"))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("$.error").value("Missing X-Hub-Signature header"));
    }
}
