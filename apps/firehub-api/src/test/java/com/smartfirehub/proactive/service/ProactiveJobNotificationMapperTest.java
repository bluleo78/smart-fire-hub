 ...
 *
 * <p>구 형식(["CHAT", "EMAIL"])과 신 형식([{type, recipientUserIds, recipientEmails}]) 모두
 * 수용, 같은 사용자에게 여러 채널은 한 Recipient에 묶이는지, 외부 이메일이 user id 없이 별도
 * Recipient로 떨어지는지 확인.
 */
class ProactiveJobNotificationMapperTest {
    ...
    void newFormat_twoChannels_singleUser_mergedToOneRecipient() {
        Map<String, Object> config = Map.of("channels", List.of(
                Map.of("type", "CHAT", "recipientUserIds", List.of(10),
                        "recipientEmails", List.of()),
                Map.of("type", "EMAIL", "recipientUserIds", List.of(10),
                        "recipientEmails", List.of())
        ));
    ...
    private ProactiveJobResponse sampleJob(Map<String, Object> config, long ownerUserId) {
        return new ProactiveJobResponse(
                42L, ownerUserId,
                null, null,             // templateId, templateName
                "Test Job", "prompt",
                null, null,             // cronExpression, timezone
                true,
                config,
                null, null,             // lastExecutedAt, nextExecuteAt
