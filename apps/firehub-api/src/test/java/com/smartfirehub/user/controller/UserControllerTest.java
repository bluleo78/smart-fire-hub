...
class UserControllerTest {
    ...
    void getUsers_withPermission_returnsList() throws Exception {
        ...
        PageResponse<UserResponse> page = new PageResponse<>(
                List.of(new UserResponse(1L, "testuser", "test@example.com", "Test User", true, LocalDateTime.now())),
                0, 20, 1, 1
        );
    ...
    void getUserById_withPermission_returnsUser() throws Exception {
        mockAuthentication("user:read");
        UserDetailResponse detail = new UserDetailResponse(
                2L, "otheruser", "other@example.com", "Other User", true, LocalDateTime.now(),
                List.of()
        );
    ...
    void setUserActive_withPermission_returnsNoContent() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/users/2/active")
                        .header("Authorization", "Bearer valid-token")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isNoContent());
