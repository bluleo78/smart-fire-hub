...
class AiControllerTest {
    ...
    @MockitoBean
    private AiAgentProxyService aiAgentProxyService;
    ...
    void getSessions_authenticated_returnsOk() throws Exception {
        ...
        AiSessionResponse session = new AiSessionResponse(1L, 1L, "session-001", null, null, "My Session", LocalDateTime.now(), LocalDateTime.now());
    ...
    void createSession_authenticated_returnsCreated() throws Exception {
        ...
        CreateAiSessionRequest request = new CreateAiSessionRequest("session-new", "dataset", 10L, "New Session");
        AiSessionResponse response = new AiSessionResponse(2L, 1L, "session-new", "dataset", 10L, "New Session", LocalDateTime.now(), LocalDateTime.now());
        when(aiSessionService.createSession(eq(1L), any(CreateAiSessionRequest.class))).thenReturn(response);
    ...
    void getSessionMessages_authenticated_returnsOk() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/ai/sessions/test-session-id/messages")
                        .header("Authorization", "Bearer valid-token"))
                .andExpect(status().isOk())
    ...
    void getSessionMessages_otherUserSession_returnsForbidden() throws Exception {
        ...
        doThrow(new AccessDeniedException("AI 세션에 대한 권한이 없습니다"))
                .when(aiSessionService).verifySessionOwnership(1L, "other-session-id");

        mockMvc.perform(get("/api/v1/ai/sessions/other-session-id/messages")
                        .header("Authorization", "Bearer valid-token"))
