 ...
 *
 * <p>Spring 컨텍스트 없이 Mockito만으로 스케줄러 정리 로직을 검증한다.
...
class AsyncJobCleanupServiceTest {
  ...
  /**
   * 정상: 스테일 잡이 있으면 각 잡에 대해 failJob이 호출되어야 한다.
   * findStaleJobs가 2개를 반환하면 asyncJobService.failJob이 2번 호출된다.
   */
  @Test
  ...
  /**
   * 엣지 케이스: 스테일 잡이 없으면 asyncJobService와 상호작용이 없어야 한다.
   * 조기 반환 경로 검증.
   */
  @Test
  ...
  /**
   * 정상: findStaleJobs 호출 시 현재 시각 기준 30분 이전 임계값이 사용되어야 한다.
   * 임계값이 현재 시각보다 이전이고 현재 시각으로부터 30~31분 이내임을 검증한다.
   */
  @Test
  ...
  /**
   * 정상: deleteOlderThan이 삭제 건수를 반환하면 정상 종료되어야 한다.
   * asyncJobRepository.deleteOlderThan이 1번 호출됨을 검증한다.
   */
  @Test
