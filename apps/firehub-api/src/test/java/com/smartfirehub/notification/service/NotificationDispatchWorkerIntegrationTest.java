...
/**
 * Dispatcher → Worker → ChatChannel 전체 흐름 통합 검증.
 * LISTEN/NOTIFY는 테스트에서 비활성화(다른 프로세스의 listener 간섭 회피), 워커를 직접 호출한다.
 */
...
class NotificationDispatchWorkerIntegrationTest extends IntegrationTestBase {
    ...
    private NotificationRequest chatRequest(long userId, UUID corr) {
        return new NotificationRequest(
                "TEST_WORKER", null, userId, corr,
                new Payload(Payload.PayloadType.STANDARD, "워커 테스트", "요약",
