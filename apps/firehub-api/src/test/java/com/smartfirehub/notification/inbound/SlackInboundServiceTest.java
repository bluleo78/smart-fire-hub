 ...
 *
 * <p>외부 의존성(SlackApiClient, AiAgentBatchClient 등)은 모두 Mockito mock으로 교체하여
 * 서비스 로직만 검증한다. @Async는 테스트에서 동기 실행되므로 별도 설정 불필요.
 */
...
class SlackInboundServiceTest {
    ...
    void setUp() {
        ...
        when(workspaceRepo.findByTeamId(TEAM_ID))
                .thenReturn(Optional.of(new SlackWorkspace(
                        1L, TEAM_ID, "TestWS", "B001", BOT_TOKEN_ENC,
                        null, null, null, null)));
        when(encryption.decrypt(BOT_TOKEN_ENC)).thenReturn(BOT_TOKEN);

        // 사용자 binding stub
        when(bindingRepo.findByExternalId(TEAM_ID, SLACK_USER))
                .thenReturn(Optional.of(new UserChannelBinding(
                        10L, USER_ID, ChannelType.SLACK, 1L, SLACK_USER,
                        "U123@TestWS", null, null, null, "ACTIVE", null,
                        Instant.now(), Instant.now())));

        // AI 세션 — 기본: 없음 (새로 생성)
        when(aiSessionRepo.findBySlackContext(TEAM_ID, CHANNEL, THREAD_TS))
                .thenReturn(Optional.empty());
        when(aiAgentClient.createSession(USER_ID, "Slack " + THREAD_TS))
                .thenReturn(AGENT_SESSION_ID);
    ...
    void dispatch_successfulFlow_createsSessionAndReplies() {
        // given
        when(aiSessionRepo.createSlackSession(anyLong(), anyString(), anyString(), anyString(), anyString(), anyString()))
                .thenReturn(99L);
        ...
        verify(aiSessionRepo).createSlackSession(
                eq(USER_ID), eq(AGENT_SESSION_ID),
                eq(TEAM_ID), eq(CHANNEL), eq(THREAD_TS), anyString());
    ...
    void dispatch_existingSessionReused() {
        // given: 이미 존재하는 세션
        AiSessionResponse existingSession = new AiSessionResponse(
                5L, USER_ID, AGENT_SESSION_ID, null, null,
                "Slack 대화", LocalDateTime.now(), LocalDateTime.now(),
                "SLACK", TEAM_ID, CHANNEL, THREAD_TS);
        when(aiSessionRepo.findBySlackContext(TEAM_ID, CHANNEL, THREAD_TS))
        ...
        verify(aiSessionRepo, never()).createSlackSession(anyLong(), anyString(), anyString(), anyString(), anyString(), anyString());
    ...
    void dispatch_unknownWorkspace_logsAndReturns() {
        ...
        verify(slackApiClient, never()).reactionsAdd(anyString(), anyString(), anyString(), anyString());
        verify(slackApiClient, never()).postEphemeral(anyString(), anyString(), anyString(), anyString());
        verify(aiAgentClient, never()).chat(anyString(), anyLong(), anyString());
        verify(slackChannel, never()).replyTo(anyLong(), anyString(), anyString(), anyString());
    }
}
