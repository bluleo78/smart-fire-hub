 ...
 * SlackChannel 단위 테스트.
 *
 * <p>SlackApiClient 대신 ChannelHttpClient를 사용하는 새 구현 검증. 주요 경로: 정상 발송, binding 누락, workspace 누락, 인증
 * 오류(401), rate limit(5xx).
 */
...
class SlackChannelTest {
  ...
  void setUp() {
    channel =
        new SlackChannel(
            channelHttpClient, renderer, bindingRepo, workspaceRepo, encryptionService);
  ...
  private SlackWorkspaceRepository.SlackWorkspace workspace() {
    return new SlackWorkspaceRepository.SlackWorkspace(
        WORKSPACE_ID, "T0123456", "Test Team", "B0123456", BOT_TOKEN_ENC, null, null, null, 1L);
  }
  ...
  /** 정상 경로: binding 존재, workspace 존재, channelHttpClient.send 성공 → Sent(slack-{outboxId}) 반환. */
  @Test
  void deliver_success_returnsSent() {
    ...
    verify(channelHttpClient).send(eq("SLACK"), any(Map.class), any(Map.class), isNull());
  }
  ...
  /**
   * channelHttpClient.send가 ChannelHttpException(401) 던질 때 TOKEN_EXPIRED PermanentFailure 반환 검증.
   */
  @Test
  ...
  /** channelHttpClient.send가 ChannelHttpException(500) 던질 때 TransientFailure 반환 검증. */
  @Test
  ...
  void replyTo_callsChannelHttpClientWithThreadTs() {
    ...
    verify(channelHttpClient)
        .send(eq("SLACK"), any(Map.class), any(Map.class), eq("1234567890.000100"));
  }
}
