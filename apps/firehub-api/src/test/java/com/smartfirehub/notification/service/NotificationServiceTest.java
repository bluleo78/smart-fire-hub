 ...
 * SseEmitterRegistry를 @MockitoBean으로 모킹하여 broadcast 호출 및 NotificationEvent 내용을 검증한다.
 * @Async 메서드(onPipelineCompleted)는 직접 호출하여 동기적으로 검증한다.
 */
class NotificationServiceTest extends IntegrationTestBase {
  ...
  /**
   * 정상: 파이프라인 COMPLETED 이벤트 수신 시 registry.broadcast()가 호출되고
   * 이벤트 타입이 PIPELINE_COMPLETED, severity가 INFO여야 한다.
   */
  @Test
  ...
  /**
   * 엣지 케이스: createdBy가 null인 이벤트는 registry.broadcast()가 호출되지 않아야 한다.
   * (알림을 보낼 대상 사용자를 특정할 수 없으므로 스킵)
   * @Async이므로 충분히 대기한 후 interaction이 없음을 검증한다.
   */
  @Test
  ...
  /**
   * 정상: 데이터셋 변경 알림 시 registry.broadcastAll()이 DATASET_CHANGED 이벤트로 호출되어야 한다.
   * 특정 사용자가 아닌 전체 브로드캐스트이므로 broadcastAll()을 사용해야 한다.
   */
  @Test
  ...
  /**
   * 정상: 각 NotificationEvent는 고유한 UUID id를 가져야 한다.
   * 두 번 호출 시 서로 다른 id가 생성되어야 한다.
   */
  @Test
