...
class NotificationDispatcherTest {
    ...
    private final NotificationOutboxRepository outboxRepo = org.mockito.Mockito.mock(NotificationOutboxRepository.class);
    ...
    private NotificationDispatcher enabled() {
        return new NotificationDispatcher(routingResolver, outboxRepo, keyGen, notifier,
                objectMapper, true);
    }
    ...
    void enqueue_insertsOneRowPerResolvedChannel() {
        ...
        ArgumentCaptor<NotificationOutboxRow> captor = ArgumentCaptor.forClass(NotificationOutboxRow.class);
    ...
    void enqueue_advisesUserWhenForcedChatFallback() {
        ...
        when(routingResolver.resolve(r)).thenReturn(new ResolvedRouting(
                ...
                Map.of(ChannelType.SLACK, "BINDING_MISSING"),
                true
        ));
    ...
    void enqueue_autoGeneratesCorrelationIdWhenNull() {
        Recipient r = new Recipient(1L, null, EnumSet.of(ChannelType.CHAT));
        NotificationRequest req = new NotificationRequest(
                "TEST_EVENT", null, 99L, null,           // null correlationId
                samplePayload(), null, List.of(r)
        );
        when(routingResolver.resolve(r)).thenReturn(new ResolvedRouting(
                List.of(ChannelType.CHAT),
