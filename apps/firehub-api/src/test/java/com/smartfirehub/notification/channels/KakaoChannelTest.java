 ...
 * KakaoChannel 단위 테스트.
 *
 * <p>KakaoApiClient 대신 ChannelHttpClient를 사용하는 새 구현 검증. 주요 경로: 정상 발송, binding 누락, 토큰 만료 status,
 * 401(TOKEN_EXPIRED), 5xx(TransientFailure).
 */
...
class KakaoChannelTest {
  ...
  void deliver_success_returnsSent() {
    ...
    verify(channelHttpClient).send(eq("KAKAO"), any(Map.class), any(Map.class));
  }
  ...
  void deliver_bindingStatusNotActive_returnsPermanentFailureTokenExpired() {
    UserChannelBinding expiredBinding =
        new UserChannelBinding(
            ...
            Instant.now());
    when(bindingRepo.findActive(USER_ID, ChannelType.KAKAO))
        .thenReturn(Optional.of(expiredBinding));
