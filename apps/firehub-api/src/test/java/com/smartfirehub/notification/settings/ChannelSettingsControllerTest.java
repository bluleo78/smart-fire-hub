 ...
 *
 * <p>JwtTokenProvider·PermissionService를 Mockito로 모킹하여 JWT 인증을 시뮬레이션한다.
 * ChannelSettingsService는 Mockito mock으로 비즈니스 로직과 분리하여 컨트롤러 레이어만 검증한다.
 */
...
class ChannelSettingsControllerTest {
    ...
    void getSettings_authenticated_returnsFourChannels() throws Exception {
        ...
        // when & then
        mockMvc.perform(get("/api/v1/channels/settings")
    ...
    void updatePreference_email_disabled_returns204() throws Exception {
        mockMvc.perform(patch("/api/v1/channels/settings/EMAIL/preference")
                        .header("Authorization", AUTH_HEADER)
    ...
    void updatePreference_chat_returns400() throws Exception {
        doThrow(new IllegalArgumentException("CHAT 채널은 항상 활성 상태이며 변경할 수 없습니다."))
                .when(channelSettingsService).updatePreference(TEST_USER_ID, ChannelType.CHAT, false);

        mockMvc.perform(patch("/api/v1/channels/settings/CHAT/preference")
                        .header("Authorization", AUTH_HEADER)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"enabled\": false}"))
    ...
    void disconnectBinding_kakao_returns204() throws Exception {
        mockMvc.perform(delete("/api/v1/channels/settings/KAKAO")
                        .header("Authorization", AUTH_HEADER))
                .andExpect(status().isNoContent());
