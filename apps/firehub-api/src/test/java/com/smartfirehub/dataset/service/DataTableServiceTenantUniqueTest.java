...
class DataTableServiceTenantUniqueTest extends IntegrationTestBase {
  ...
  void twoTenantsCanUseSameTableName() {
    long a = TENANT_BASE + 1;
    long b = TENANT_BASE + 2;
    TenantRlsTestSupport.insertActiveTenant(dsl, a);
    TenantRlsTestSupport.insertActiveTenant(dsl, b);
    String schemaA = TenantContext.runScopedGet(a, DataSchema::current);
    String schemaB = TenantContext.runScopedGet(b, DataSchema::current);
    Long userA = TenantRlsTestSupport.insertUser(dsl, "t5-unique-a");
    Long userB = TenantRlsTestSupport.insertUser(dsl, "t5-unique-b");

    try {
      TenantContext.runScopedGet(
          a, () -> datasetService.createDataset(newRequest("공유이름-A", "shared_name"), userA));
      // V112 접기 전이면 여기서 카탈로그 유니크 위반(23505)으로 터진다.
      TenantContext.runScopedGet(
          b, () -> datasetService.createDataset(newRequest("공유이름-B", "shared_name"), userB));
      ...
      TenantRlsTestSupport.cleanupAll(
          ...
          () -> deleteOwnDatasetRows(a),
          () -> deleteOwnDatasetRows(b),
          ...
          () -> deleteOwnAuditLogRows(a),
          () -> deleteOwnAuditLogRows(b),
          () -> TenantRlsTestSupport.dropSchemasCreatedByThisTest(ownerDsl(), schemaA, schemaB),
          () -> TenantRlsTestSupport.deleteTenants(dsl, a, b),
  ...
  void recreatingOneTenantsTableLeavesTheOthersRowsIntact() {
    long a = TENANT_BASE + 3;
    long b = TENANT_BASE + 4;
    TenantRlsTestSupport.insertActiveTenant(dsl, a);
    TenantRlsTestSupport.insertActiveTenant(dsl, b);
    String schemaA = TenantContext.runScopedGet(a, DataSchema::current);
    String schemaB = TenantContext.runScopedGet(b, DataSchema::current);
    Long userA = TenantRlsTestSupport.insertUser(dsl, "t5-intact-a");
    Long userB = TenantRlsTestSupport.insertUser(dsl, "t5-intact-b");
    ...
    try {
      ...
      TenantContext.runScopedGet(
          b, () -> datasetService.createDataset(newRequest("교체검증-B", "shared_name"), userB));

      // 핵심 단언: A 의 행이 살아 있다 — 두 테넌트가 물리적으로 다른 테이블
      // (data_t{a}."shared_name" 와 data_t{b}."shared_name")을 갖기 때문이다.
      List<String> aRows = TenantContext.runScopedGet(a, () -> selectAllValues("shared_name"));
      assertThat(aRows).containsExactly("tenant-a-row");
      // 그리고 B 의 테이블은 비어 있다(A 것을 넘겨받은 게 아니다).
      assertThat(TenantContext.runScopedGet(b, () -> selectAllValues("shared_name"))).isEmpty();
    } finally {
      TenantRlsTestSupport.cleanupAll(
          // dataset 카탈로그 행을 먼저 지운다 — fk_dataset_tenant 에 ON DELETE CASCADE 가 없어
          // deleteTenants 가 이 행보다 먼저 tenant 를 지우려 하면 FK 위반으로 실패한다.
  ...
  /**
   * 이 테스트가 만든 {@code dataset} 카탈로그 행을 지운다(RLS 스코프라 해당 테넌트 행만 지워진다).
   * {@code dataset_column} 은 {@code ON DELETE CASCADE} 로 함께 사라진다. {@code deleteTenants}
   * 보다 반드시 먼저 호출해야 한다 — {@code fk_dataset_tenant} 에는 cascade 가 없다.
   */
   ...
   * user·tenant 삭제가 FK 위반으로 실패한다({@code TriggerEventServiceTest} 의 같은 패턴 참조).
   */
