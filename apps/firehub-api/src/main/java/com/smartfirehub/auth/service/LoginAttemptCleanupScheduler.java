 ...
 *
 * <p>lazy expiry(`expires_at > now()` 필터)만으로도 정확성은 보장되지만, 누적된 만료 row가 인덱스 크기를 키우는 것을 막기 위해
 * 1시간 주기로 일괄 삭제한다. 테스트 컨텍스트에서는 {@code app.login-attempts.cleanup.enabled=false}로 비활성화한다.
 */
@Slf4j
@Component
