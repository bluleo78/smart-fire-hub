 ...
 * API 연결 헬스체크 주기 실행기.
 *
 * <p>10분마다 health_check_path가 설정된 API 연결을 순회하며 상태를 갱신하고,
 * 상태 전환(UP↔DOWN) 발생 시 ApiConnectionNotifier로 알림을 디스패치한다.
 *
 * <p>initialDelay=60s: 애플리케이션 기동 직후 외부 API 호출을 방지하기 위해
 * 첫 실행을 1분 뒤로 지연한다.
 */
@Component
...
public class ApiConnectionHealthCheckScheduler {
   ...
   *
   * <p>각 연결을 독립적으로 처리하여 한 연결의 실패가 나머지 순회를 중단시키지 않도록
   * try-catch로 격리한다.
   */
