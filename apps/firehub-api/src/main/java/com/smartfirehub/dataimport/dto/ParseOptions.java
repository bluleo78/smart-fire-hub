...
/**
 * CSV 파싱 옵션. 사용자가 명시적으로 인코딩을 지정하지 않으면 {@code AUTO}로 두고
 * {@link #detectEncoding(byte[])}가 BOM과 CharsetDecoder strict 검증을 통해 결정한다.
 */
public record ParseOptions(
    String delimiter, // ",", "\t", ";", "|", or custom single char. Default ","
    String encoding, // "AUTO", "UTF-8", "UTF-16LE", "UTF-16BE", "EUC-KR", "CP949", "MS949". Default "AUTO"
   ...
   *   <li>{@link CharsetDecoder}로 strict UTF-8 디코딩 성공 → UTF-8.
   *       {@code endOfInput=false}로 호출하므로 peek 버퍼 끝에서 멀티바이트 시퀀스가
   *       잘려 있어도 MALFORMED이 아닌 UNDERFLOW로 처리되어 유효 판정된다.
   *       (이전 구현은 truncation을 invalid로 보고 EUC-KR로 잘못 폴백하던 회귀가 있었다 — #263)
   ...
   *
   * <p>핵심: {@code decode(in, out, false)}로 호출하여 마지막에 멀티바이트 시퀀스가
   * 잘려 있어도 MALFORMED이 아닌 UNDERFLOW로 처리되도록 한다 (peek 버퍼 경계 truncation 대응).
   * 디코딩 도중 한 곳이라도 MALFORMED/UNMAPPABLE이면 UTF-8이 아니라고 판정한다.
   *
