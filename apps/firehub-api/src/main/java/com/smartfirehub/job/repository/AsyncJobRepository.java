...
public class AsyncJobRepository {
  ...
  private static final Field<String> AJ_JOB_TYPE =
      field(name("async_job", "job_type"), String.class);
  private static final Field<String> AJ_RESOURCE =
      field(name("async_job", "resource"), String.class);
  private static final Field<String> AJ_RESOURCE_ID =
      field(name("async_job", "resource_id"), String.class);
  ...
  private static final Field<Integer> AJ_PROGRESS =
      field(name("async_job", "progress"), Integer.class);
  ...
  private static final Field<String> AJ_ERROR_MESSAGE =
      field(name("async_job", "error_message"), String.class);
  private static final Field<LocalDateTime> AJ_CREATED_AT =
  ...
  public void updateStageAndProgress(
      String id, String stage, int progress, String message, Map<String, Object> metadata) {
  ...
  public List<AsyncJobStatusResponse> findActiveByResource(
  ...
  public List<AsyncJobStatusResponse> findStaleJobs(LocalDateTime before) {
    return dsl.select(
            ...
            AJ_UPDATED_AT)
        .from(ASYNC_JOB)
        .where(AJ_UPDATED_AT.lt(before).and(AJ_STAGE.notIn("COMPLETED", "FAILED")))
        .fetch(this::mapToResponse);
  }

  public int deleteOlderThan(LocalDateTime before) {
    return dsl.deleteFrom(ASYNC_JOB)
        .where(AJ_CREATED_AT.lt(before).and(AJ_STAGE.in("COMPLETED", "FAILED")))
        .execute();
  }

  public Optional<Long> findOwnerById(String id) {
    return dsl.select(AJ_USER_ID).from(ASYNC_JOB).where(AJ_ID.eq(id)).fetchOptional(AJ_USER_ID);
  }
}
