 ...
 *
 * <p><b>fail-closed 계약의 신호 타입이다.</b> 알 수 없는 유형을 만났다고 플랫폼 값으로 조용히
 * 폴백하면, 테넌트가 고른 실행 형태가 아니라 플랫폼 기본값으로 과금이 새는 회귀가 재발한다
 * ({@code 6b1c6383} 과 같은 모양). 그래서 {@link AiCredentialDocument} 를 실제로 소비해 타입
