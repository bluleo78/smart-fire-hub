...
public class UserRepository {
    ...
    public Optional<UserResponse> findByUsername(String username) {
        return dsl.select(USER.ID, USER.USERNAME, USER.EMAIL, USER.NAME, USER.IS_ACTIVE, USER.CREATED_AT)
                .from(USER)
    ...
    public boolean existsByUsername(String username) {
        return dsl.fetchExists(
                dsl.selectOne().from(USER).where(USER.USERNAME.eq(username))
        );
    }

    public boolean existsByEmail(String email) {
        return dsl.fetchExists(
                dsl.selectOne().from(USER).where(USER.EMAIL.eq(email))
        );
    }
    ...
    public boolean existsById(Long id) {
        return dsl.fetchExists(
                dsl.selectOne().from(USER).where(USER.ID.eq(id))
        );
    }
    ...
    public List<UserResponse> findAllPaginated(String search, int page, int size) {
        ...
        if (search != null && !search.isBlank()) {
            ...
            condition = condition.and(
                    USER.USERNAME.likeIgnoreCase(pattern)
