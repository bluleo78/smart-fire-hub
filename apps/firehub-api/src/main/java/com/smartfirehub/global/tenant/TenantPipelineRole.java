 ...
 *
 * <p><b>왜 비밀번호를 저장하지 않고 매번 파생하는가.</b> 비밀번호를 애플리케이션 저장소(DB·설정)에
 * 별도로 저장하면 "DB 롤을 만들 때 정한 값"과 "앱이 접속할 때 읽는 값" 이라는 두 개의 동기화
 * 지점이 생기고, 둘이 어긋나면 인증 실패로만 드러나 원인 추적이 어렵다. 결정적 파생(HMAC)을 쓰면
 * 두 지점이 같은 함수로 같은 값을 계산하므로 동기화 지점이 애초에 존재하지 않는다.
 * {@code RolePasswordSyncCallback}(T2) 이 Flyway 콜백에서 이 값으로 {@code ALTER ROLE ... PASSWORD}
 * 를 실행해 DB 쪽 비밀번호를 앱 기동 시마다 동일한 값으로 맞춘다.
 *
 * <p>(정정, #680) 원래 이 문단은 근거를 "테넌트 생성은 R6 에 따라 운영자가 실행하는 SQL 스크립트
 * 이지 애플리케이션 API 가 아니다" 로 적었다. 그 전제는 더 이상 참이 아니다 — 롤 생성은
 * {@link TenantPipelineRoleProvisioner} 가 한다. 결정적 파생이라는 결론은 그대로이고(오히려
 * 생성 시점부터 진짜 비밀번호를 쓸 수 있게 해 준 것이 이 파생이다) 근거만 낡았다.
 *
