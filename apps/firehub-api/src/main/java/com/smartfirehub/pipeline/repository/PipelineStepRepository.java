...
public class PipelineStepRepository {
    ...
    private static final Field<String> PS_DESCRIPTION = field(name("pipeline_step", "description"), String.class);
    private static final Field<String> PS_SCRIPT_TYPE = field(name("pipeline_step", "script_type"), String.class);
    ...
    private static final Field<Long> PSI_DATASET_ID = field(name("pipeline_step_input", "dataset_id"), Long.class);

    private static final Table<?> PIPELINE_STEP_DEPENDENCY = table(name("pipeline_step_dependency"));
    private static final Field<Long> PSD_STEP_ID = field(name("pipeline_step_dependency", "step_id"), Long.class);
    private static final Field<Long> PSD_DEPENDS_ON_STEP_ID = field(name("pipeline_step_dependency", "depends_on_step_id"), Long.class);
    ...
    public List<PipelineStepResponse> findByPipelineId(Long pipelineId) {
        ...
        var steps = dsl.select(
                ...
                .from(PIPELINE_STEP)
                .leftJoin(DATASET).on(PS_OUTPUT_DATASET_ID.eq(D_ID))
    ...
    public void deleteByPipelineId(Long pipelineId) {
        ...
        if (!stepIds.isEmpty()) {
            // Delete dependencies
            dsl.deleteFrom(PIPELINE_STEP_DEPENDENCY)
                    .where(PSD_STEP_ID.in(stepIds))
                    .execute();

            // Delete inputs
            dsl.deleteFrom(PIPELINE_STEP_INPUT)
                    .where(PSI_STEP_ID.in(stepIds))
                    .execute();
