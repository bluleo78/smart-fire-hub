...
public class TriggerEventRepository {
    ...
    private static final Field<Long> TE_TRIGGER_ID = field(name("trigger_event", "trigger_id"), Long.class);
    private static final Field<Long> TE_PIPELINE_ID = field(name("trigger_event", "pipeline_id"), Long.class);
    private static final Field<Long> TE_EXECUTION_ID = field(name("trigger_event", "execution_id"), Long.class);
    private static final Field<String> TE_EVENT_TYPE = field(name("trigger_event", "event_type"), String.class);
    private static final Field<JSONB> TE_DETAIL = field(name("trigger_event", "detail"), JSONB.class);
    private static final Field<LocalDateTime> TE_CREATED_AT = field(name("trigger_event", "created_at"), LocalDateTime.class);
    ...
    public void create(Long triggerId, Long pipelineId, Long executionId, String eventType, Map<String, Object> detail) {
        var query = dsl.insertInto(TRIGGER_EVENT)
    ...
    public List<TriggerEventResponse> findByPipelineId(Long pipelineId, int limit) {
        return dsl.select(TE_ID, TE_TRIGGER_ID, T_NAME, TE_EVENT_TYPE, TE_EXECUTION_ID, TE_DETAIL, TE_CREATED_AT)
                .from(TRIGGER_EVENT)
                .leftJoin(PIPELINE_TRIGGER).on(TE_TRIGGER_ID.eq(T_ID))
                .where(TE_PIPELINE_ID.eq(pipelineId))
                .orderBy(TE_CREATED_AT.desc())
                .limit(limit)
                .fetch(r -> new TriggerEventResponse(
                        r.get(TE_ID),
                        r.get(TE_TRIGGER_ID),
                        r.get(T_NAME),
                        r.get(TE_EVENT_TYPE),
