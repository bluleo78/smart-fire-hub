...
public class QueryHistoryRepository {
    ...
    private static final Field<Long> QH_DATASET_ID = field(name("query_history", "dataset_id"), Long.class);
    private static final Field<Long> QH_USER_ID = field(name("query_history", "user_id"), Long.class);
    private static final Field<String> QH_SQL_TEXT = field(name("query_history", "sql_text"), String.class);
    private static final Field<String> QH_QUERY_TYPE = field(name("query_history", "query_type"), String.class);
    private static final Field<Integer> QH_AFFECTED_ROWS = field(name("query_history", "affected_rows"), Integer.class);
    private static final Field<Long> QH_EXECUTION_TIME_MS = field(name("query_history", "execution_time_ms"), Long.class);
    private static final Field<Boolean> QH_SUCCESS = field(name("query_history", "success"), Boolean.class);
    private static final Field<String> QH_ERROR_MESSAGE = field(name("query_history", "error_message"), String.class);
    private static final Field<LocalDateTime> QH_EXECUTED_AT = field(name("query_history", "executed_at"), LocalDateTime.class);

    public QueryHistoryRepository(DSLContext dsl) {
        this.dsl = dsl;
    }

    public void save(Long datasetId, Long userId, String sql, String queryType,
                     int affectedRows, long executionTimeMs, boolean success, String error) {
    ...
    public List<QueryHistoryResponse> findByDatasetId(Long datasetId, int page, int size) {
        return dsl.select(QH_ID, QH_SQL_TEXT, QH_QUERY_TYPE, QH_AFFECTED_ROWS,
                        QH_EXECUTION_TIME_MS, QH_SUCCESS, QH_ERROR_MESSAGE, QH_EXECUTED_AT)
                .from(QUERY_HISTORY)
