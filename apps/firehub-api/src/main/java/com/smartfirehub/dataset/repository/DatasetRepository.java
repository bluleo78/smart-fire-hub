...
public class DatasetRepository {
    ...
    private static final Field<String> DS_STATUS_NOTE = field(name("dataset", "status_note"), String.class);
    private static final Field<Long> DS_STATUS_UPDATED_BY = field(name("dataset", "status_updated_by"), Long.class);
    private static final Field<LocalDateTime> DS_STATUS_UPDATED_AT = field(name("dataset", "status_updated_at"), LocalDateTime.class);
    ...
    private static final Field<Long> DF_USER_ID = field(name("dataset_favorite", "user_id"), Long.class);
    private static final Field<Long> DF_DATASET_ID = field(name("dataset_favorite", "dataset_id"), Long.class);
    ...
    private DatasetResponse mapToDatasetResponse(Record r, Set<Long> favoriteIds, Map<Long, List<String>> tagsByDatasetId) {
        ...
        List<String> tags = tagsByDatasetId != null ? tagsByDatasetId.getOrDefault(id, List.of()) : List.of();

        String statusUpdatedByName = null;
        Long statusUpdatedById = r.get(DS_STATUS_UPDATED_BY);
        if (statusUpdatedById != null) {
            statusUpdatedByName = dsl.select(U_NAME)
                    .from(USER_TABLE)
    ...
    private Set<Long> fetchFavoriteIds(Long currentUserId) {
        ...
        return new HashSet<>(dsl.select(DF_DATASET_ID)
                .from(DATASET_FAVORITE)
                .where(DF_USER_ID.eq(currentUserId))
                .fetch(r -> r.get(DF_DATASET_ID)));
    }

    private Condition buildCondition(Long categoryId, String datasetType, String search, String status) {
    ...
    public List<DatasetResponse> findAll(Long categoryId, String datasetType, String search, int page, int size,
        ...
        var idQuery = dsl.selectDistinct(DS_ID)
                .from(DATASET)
                .leftJoin(DATASET_COLUMN).on(COL_DATASET_ID.eq(DS_ID));

        if (favoriteOnly && currentUserId != null) {
            idQuery = idQuery.innerJoin(DATASET_FAVORITE)
                    .on(DF_DATASET_ID.eq(DS_ID).and(DF_USER_ID.eq(currentUserId)));
        }

        List<Long> datasetIds = idQuery
                .where(condition)
                .orderBy(DS_ID.asc())
                .limit(size)
                .offset(page * size)
        ...
        return dsl.select(DS_ID, DS_NAME, DS_TABLE_NAME, DS_DESCRIPTION, DS_DATASET_TYPE,
                        DS_CREATED_AT, DS_STATUS, DS_STATUS_NOTE, DS_STATUS_UPDATED_BY, DS_STATUS_UPDATED_AT,
    ...
    public long count(Long categoryId, String datasetType, String search, Long currentUserId, String status, boolean favoriteOnly) {
        Condition condition = buildCondition(categoryId, datasetType, search, status);

        var countQuery = dsl.select(countDistinct(DS_ID))
                .from(DATASET)
                .leftJoin(DATASET_COLUMN).on(COL_DATASET_ID.eq(DS_ID));

        if (favoriteOnly && currentUserId != null) {
            return countQuery
                    .innerJoin(DATASET_FAVORITE).on(DF_DATASET_ID.eq(DS_ID).and(DF_USER_ID.eq(currentUserId)))
                    .where(condition)
    ...
    public Optional<DatasetResponse> findById(Long id, Long currentUserId) {
        ...
        return dsl.select(DS_ID, DS_NAME, DS_TABLE_NAME, DS_DESCRIPTION, DS_DATASET_TYPE, DS_CREATED_AT,
                        DS_STATUS, DS_STATUS_NOTE, DS_STATUS_UPDATED_BY, DS_STATUS_UPDATED_AT,
                        DC_ID, DC_NAME, DC_DESCRIPTION)
                .from(DATASET)
                .leftJoin(DATASET_CATEGORY).on(DS_CATEGORY_ID.eq(DC_ID))
                .where(DS_ID.eq(id))
    ...
    public DatasetResponse save(CreateDatasetRequest request, Long createdBy) {
        return dsl.insertInto(DATASET)
                ...
                .returning(DS_ID, DS_NAME, DS_TABLE_NAME, DS_DESCRIPTION, DS_DATASET_TYPE, DS_CREATED_AT,
                        DS_STATUS, DS_STATUS_NOTE, DS_STATUS_UPDATED_BY, DS_STATUS_UPDATED_AT, DS_CATEGORY_ID)
                .fetchOne(r -> {
