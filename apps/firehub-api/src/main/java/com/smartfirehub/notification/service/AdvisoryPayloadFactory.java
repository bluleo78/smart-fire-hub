...
/**
 * CHAT 강제 폴백 시 사용자에게 보일 안내 메시지 생성.
 * 스펙 6장: 외부 채널이 모두 OPTED_OUT 또는 BINDING_MISSING일 때 CHAT 행과 함께 enqueue.
 */
class AdvisoryPayloadFactory {
    ...
    static Payload build(Map<ChannelType, String> skippedReasons) {
        ...
        return new Payload(
                ...
                "🔔 알림 채널 안내",
                summary.toString(),
                List.of(new Payload.Section(
                        "연동/설정 변경",
                        "채널 연동 또는 수신 설정을 변경하려면 설정 페이지에서 진행하세요.")),
