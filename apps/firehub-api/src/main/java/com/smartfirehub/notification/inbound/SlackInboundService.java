 ...
 *
 * <p>SlackEventsController가 3초 내 HTTP 200 ack를 반환한 후,
 * {@code @Async("slackInboundExecutor")} 스레드 풀에서 실제 처리를 수행한다.
 *
 * <p>처리 흐름:
 * <ol>
 *   <li>워크스페이스 조회 + 봇 토큰 복호화</li>
 *   <li>reactions.add(:eyes:) — 처리 중 표시 (실패 무시)</li>
 *   <li>user 매핑 — findByExternalId로 Smart Fire Hub 사용자 특정</li>
 *   <li>ai_session lookup/create — 동일 스레드 재진입 시 기존 세션 재사용</li>
 *   <li>ai-agent chat 호출 (60초 blocking)</li>
 *   <li>SlackChannel.replyTo — 동일 스레드에 AI 응답 전송</li>
 * </ol>
 */
@Service
    ...
    public void dispatch(String teamId, JsonNode event) {
        ...
        try {
            ...
            if (existing.isPresent()) {
                ...
                log.debug("slack inbound — 기존 세션 재사용 (sessionId={}, threadTs={})",
                        agentSessionId, threadTs);
            } else {
                // 새 스레드 시작 — ai-agent에 세션 생성 후 DB에 기록
                agentSessionId = aiAgentClient.createSession(userId, "Slack " + threadTs);
                aiSessionRepo.createSlackSession(userId, agentSessionId,
                        teamId, channel, threadTs, "Slack 대화");
                log.debug("slack inbound — 새 세션 생성 (sessionId={}, threadTs={})",
                        agentSessionId, threadTs);
            }
            ...
            try {
                ...
                } catch (Exception ignore) { /* best-effort, 실패 무시 */ }
                slackApiClient.postEphemeral(botToken, channel, slackUserId,
            ...
            log.info("slack inbound — 응답 완료 (team={}, ts={}, sessionId={})",
                    teamId, ts, agentSessionId);
