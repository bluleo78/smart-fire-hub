 ...
 *
 * <p>firehub-channel 서비스에 메시지 발송을 위임한다 (ChannelHttpClient 경유). 발송 흐름: 1. UserChannelBinding 조회
 * (workspaceId + externalUserId 필수) 2. SlackWorkspace에서 봇 토큰 복호화 3. Block Kit JSON + fallback 텍스트
 * 렌더링 4. channelHttpClient.send("SLACK", recipient, message) 위임
 *
...
public class SlackChannel implements BoundChannel {
   ...
   * Slack DM 발송.
   *
   * <p>처리 순서: 1. 활성 binding 조회 → 없으면 BINDING_REQUIRED PermanentFailure 2. binding의
   * workspaceId/externalUserId null 체크 → 없으면 BINDING_REQUIRED PermanentFailure 3. SlackWorkspace 조회
   * → 없으면 UNRECOVERABLE PermanentFailure (워크스페이스 취소됨) 4. 봇 토큰 복호화 5. Block Kit 렌더링 6.
   * channelHttpClient.send("SLACK", recipient, message) 위임 7. ChannelHttpException(401) →
   * TOKEN_EXPIRED, 기타 예외 → TransientFailure
   */
  @Override
  public DeliveryResult deliver(DeliveryContext ctx) {
    ...
    if (binding.workspaceId() == null || binding.externalUserId() == null) {
      log.warn(
          "SlackChannel: binding 필드 누락 (userId={}, workspaceId={}, externalUserId={})",
          userId,
          binding.workspaceId(),
    ...
    if (workspaceOpt.isEmpty()) {
      log.warn(
          "SlackChannel: workspace 없음 (userId={}, workspaceId={})", userId, binding.workspaceId());
      return new DeliveryResult.PermanentFailure(
    ...
    try {
      ...
      log.warn(
          "SlackChannel: 봇 토큰 복호화 실패 (userId={}, workspaceId={})",
          userId,
          binding.workspaceId(),
          e);
      return new DeliveryResult.PermanentFailure(
    ...
    Map<String, Object> recipient =
        Map.of("slackBotToken", botToken, "slackChannelId", binding.externalUserId());
    ...
    try {
      ...
      log.warn(
          "SlackChannel: 발송 오류 {} (userId={}, outboxId={})",
          e.getStatusCode(),
          userId,
          ctx.outboxId());
   ...
   *
   * <p>firehub-channel 서비스에 SLACK send 요청을 위임한다. threadTs를 포함하여 스레드 내 회신으로 처리된다.
   *
   ...
   *
   * <p>토큰 갱신이 필요하면 워크스페이스 관리자가 OAuth 재설치 필요. 여기서는 항상 StillValid를 반환하여 refresh 로직을 건너뛴다.
   *
