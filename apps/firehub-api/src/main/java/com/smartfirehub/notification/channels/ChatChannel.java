 ...
 * 웹 인박스(CHAT) 채널 구현.
 *
 * <p>proactive_message 테이블에 행 1개 INSERT 후 SseEmitterRegistry를 통해 해당 사용자에게 실시간 SSE
 * broadcast. 안전망 채널이므로 authStrategy=NONE, binding 불필요.
 *
 * <p>executionId는 Payload.metadata에 들어있으면 그대로 사용(Proactive 발송 흐름), 없으면 null —
 * message_type=REPORT는 기본값으로 유지. htmlContent 등 대용량 필드는 저장하지 않음(인박스 UI는 title/summary만 필요).
 */
@Component
public class ChatChannel implements Channel {
    ...
    public DeliveryResult deliver(DeliveryContext ctx) {
        ...
        try {
            ...
            log.error("ChatChannel INSERT failed: outboxId={} userId={}",
                    ctx.outboxId(), ctx.recipientUserId(), e);
        ...
        try {
            ...
            log.warn("ChatChannel SSE broadcast failed (메시지 저장은 성공): outboxId={} userId={}",
                    ctx.outboxId(), ctx.recipientUserId(), e);
        }
