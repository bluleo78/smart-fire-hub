 ...
 * @param workspaceId SLACK 전용 — 워크스페이스 OAuth 설치가 완료되었으나 사용자 binding이 없을 때, 프론트엔드가 사용자 ID 매핑 입력 UI를
 *     노출하기 위한 식별자. SLACK 채널이고 활성 워크스페이스가 존재하면 값이 채워지고, 그 외(설치 전·다른 채널)는 null. {@code POST
