 ...
 *
 * <p>SKIP LOCKED + lease 컬럼으로 멀티 인스턴스 안전. Sent/TransientFailure/PermanentFailure
 * sealed result로 완전 분기. feature flag OFF 상태에서는 no-op.
 *
...
public class NotificationDispatchWorker {
        ...
        for (var row : rows) {
            try {
                ...
                if (backoff.exhausted(next)) {
                    outboxRepo.markPermanentFailure(row.id(), "UNRECOVERABLE",
    ...
    private void deliverOne(NotificationOutboxRow row) {
        ...
        try {
            ...
            outboxRepo.markPermanentFailure(row.id(), "RECIPIENT_INVALID",
                    "payload parse: " + e.getMessage());
            return;
        }
        if (payload == null) {
            outboxRepo.markPermanentFailure(row.id(), "RECIPIENT_INVALID",
        ...
        try {
            ...
            result = new DeliveryResult.TransientFailure(
        ...
        switch (result) {
            ...
            case DeliveryResult.PermanentFailure pf -> outboxRepo.markPermanentFailure(
                    row.id(), pf.reason().name(), pf.details());
        }

        if (log.isDebugEnabled()) {
            log.debug("deliver outboxId={} channel={} result={}",
                    row.id(), row.channelType(), result.getClass().getSimpleName());
        }
    }
}
