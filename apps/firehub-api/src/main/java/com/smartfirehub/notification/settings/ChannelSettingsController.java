 ...
 * 사용자 채널 설정 API 컨트롤러.
 *
 * <p>인증된 사용자가 자신의 채널별 알림 수신 설정 조회·변경·연동 해제를 수행한다.
 * 모든 엔드포인트는 JWT 인증 필요 (Security Filter에서 처리).
 *
 * <p>GET  /api/v1/channels/settings           — 전체 채널 설정 조회
 * PATCH /api/v1/channels/settings/{channel}/preference — 알림 수신 여부 변경
 * DELETE /api/v1/channels/settings/{channel} — 외부 binding 해제
 */
@RestController
