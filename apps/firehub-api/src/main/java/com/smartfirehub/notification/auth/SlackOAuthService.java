 ...
 *
 * <p>관리자가 Slack 앱을 워크스페이스에 1회 설치하면 봇 토큰을 slack_workspace 테이블에 저장한다.
 * 이후 각 사용자가 자신의 Slack user ID를 수동 입력하면 봇이 DM ping을 전송하여 연동을 확인한다.
 *
...
public class SlackOAuthService {
    ...
    @Value("${notification.slack.scopes:chat:write,im:write,im:history,users:read,reactions:write,app_mentions:read}")
     ...
     *
     * <p>관리자가 새 창으로 열어 Slack 앱 설치를 진행한다.
     * scope에는 봇 운영에 필요한 권한이 모두 포함된다.
     *
     ...
     *
     * <p>응답의 ok=false 이면 IllegalStateException을 던진다.
     * 봇 토큰은 AES-256-GCM으로 암호화하여 DB에 upsert한다.
     * 이미 설치된 워크스페이스라면 ON CONFLICT로 갱신(재설치)한다.
     *
    ...
    public SlackWorkspaceInstalled completeAuthorization(String code, long installedByUserId) {
        ...
        long workspaceId = slackWorkspaceRepo.upsertFromOAuth(
     ...
     *
     * <p>워크스페이스 봇 토큰을 복호화하여 Slack API를 호출한다.
     * - users.info로 display name을 조회한다.
     * - conversations.open으로 DM 채널 ID를 획득한다.
     * - chat.postMessage로 연동 확인 DM을 발송한다.
     * 최종적으로 user_channel_binding 테이블에 upsert한다.
     *
    ...
    public void linkUser(long userId, long workspaceId, String slackUserId) {
        ...
        SlackWorkspaceRepository.SlackWorkspace workspace = slackWorkspaceRepo.findById(workspaceId)
                .orElseThrow(() -> new IllegalArgumentException(
                        "slack workspace not found: " + workspaceId));
        ...
        slackApiClient.chatPostMessage(
                ...
                dmChannelId,
                null,
                "Smart Fire Hub 채널 연동이 확인되었습니다.");

        // user_channel_binding upsert
        Instant now = Instant.now();
        userChannelBindingRepo.upsert(new UserChannelBinding(
                null,
                userId,
                ChannelType.SLACK,
                workspaceId,
     ...
     */
    public record SlackWorkspaceInstalled(
            String teamId,
            String teamName,
            long workspaceId) {}
}
