 ...
 * Slack Block Kit 렌더러.
 *
 * <p>StandardPayload를 Slack Block Kit JSON 배열 문자열로 변환한다.
 * 구성 순서: header(title) → section(summary) → sections 항목 → actions(links).
 *
 * <p>길이 제한 방어: summary 2000자, sections 20개, links 5개.
 * Slack blocks 배열은 최대 50 요소, 각 텍스트 3000자 제한이 있으나 위 제한으로 충분히 커버된다.
 */
@Component
public class SlackBlockKitRenderer {
     ...
     *
     * <p>Slack push notification과 접근성 도구에서 표시되는 텍스트.
     * title + "\n" + summary, 500자 트렁케이트.
     *
     * @param payload 발송 페이로드
