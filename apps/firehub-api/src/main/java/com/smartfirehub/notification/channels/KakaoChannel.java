 ...
 * firehub-channel이 카카오 "나에게 보내기" API를 직접 호출하며 인증 오류(401)를 반환한다.
 *
...
public class KakaoChannel implements BoundChannel {
     ...
     *
     * <p>처리 순서:
     * 1. 활성 binding 조회 → 없으면 BINDING_REQUIRED
     * 2. binding status != ACTIVE → TOKEN_EXPIRED
     * 3. access_token 복호화 → 텍스트 렌더 → channelHttpClient.send("KAKAO", ...) 위임
     * 4. ChannelHttpException(401) → binding TOKEN_EXPIRED 마킹 + PermanentFailure
     * 5. ChannelHttpException(5xx) → TransientFailure
     */
    @Override
    public DeliveryResult deliver(DeliveryContext ctx) {
        ...
        try {
            ...
            log.warn("KakaoChannel: 발송 오류 {} (userId={}, outboxId={})",
                    e.getStatusCode(), userId, ctx.outboxId());
