 ...
 *
 * <p>수신자 × requested_channels 펼치기 → RoutingResolver로 resolved_channels 확정 →
 * 멱등성 키로 outbox INSERT → NOTIFY 발행. CHAT 강제 폴백 시 안내 메시지 1건 추가.
 *
 * <p>feature flag notification.outbox.enabled=false인 경우 no-op (회귀 안전).
 * 도메인 코드는 flag를 별도로 확인하고 기존 직접 호출 경로를 유지할 책임.
 */
@Service
public class NotificationDispatcher {
    ...
    /**
     * 도메인이 호출. 멱등성 키 기반 INSERT로 중복 발송 차단.
     * feature flag OFF면 즉시 반환 (호출자가 기존 직접 호출 경로를 사용).
     */
    public void enqueue(NotificationRequest request) {
        ...
        UUID correlationId = request.correlationId() == null ? UUID.randomUUID()
        ...
        for (Recipient recipient : request.recipients()) {
            ...
            for (ChannelType channel : routing.resolvedChannels()) {
                outboxRepo.insertIfAbsent(buildRow(request, recipient, channel,
                                                   correlationId, request.standardPayload()));
            }
    ...
    private NotificationOutboxRow buildAdvisoryRow(NotificationRequest req, Recipient r,
                                                   UUID correlationId,
                                                   Map<ChannelType, String> reasons) {
