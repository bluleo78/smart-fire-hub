...
/**
 * AI 세션 레포지토리.
 * jOOQ DSLContext 기반 type-safe SQL로 ai_session 테이블을 관리한다.
 * WEB(기본) 및 SLACK 채널 소스를 지원한다.
 */
@Repository
...
public class AiSessionRepository {
    ...
    private static final Field<String> CHANNEL_SOURCE    = field(name("ai_session", "channel_source"),    String.class);
    private static final Field<String> SLACK_TEAM_ID     = field(name("ai_session", "slack_team_id"),     String.class);
    private static final Field<String> SLACK_CHANNEL_ID  = field(name("ai_session", "slack_channel_id"),  String.class);
    private static final Field<String> SLACK_THREAD_TS   = field(name("ai_session", "slack_thread_ts"),   String.class);

    public List<AiSessionResponse> findByUserId(Long userId) {
        return dsl.select(
                    ID, USER_ID, SESSION_ID, CONTEXT_TYPE, CONTEXT_RESOURCE_ID,
                    TITLE, CREATED_AT, UPDATED_AT)
                .from(AI_SESSION)
                .where(USER_ID.eq(userId))
                .orderBy(UPDATED_AT.desc())
                .fetch(r -> AiSessionResponse.ofWeb(
                        r.get(ID), r.get(USER_ID), r.get(SESSION_ID),
                        r.get(CONTEXT_TYPE), r.get(CONTEXT_RESOURCE_ID),
                        r.get(TITLE), r.get(CREATED_AT), r.get(UPDATED_AT)));
    }
    ...
    public Optional<AiSessionResponse> findById(Long id) {
        return dsl.select(
                ...
                .from(AI_SESSION)
                .where(ID.eq(id))
                .fetchOptional(r -> AiSessionResponse.ofWeb(
                        r.get(ID), r.get(USER_ID), r.get(SESSION_ID),
                        r.get(CONTEXT_TYPE), r.get(CONTEXT_RESOURCE_ID),
                        r.get(TITLE), r.get(CREATED_AT), r.get(UPDATED_AT)));
    }
    ...
    /**
     * SLACK 스레드 기준 세션 조회.
     * (slack_team_id, slack_channel_id, slack_thread_ts) 3-tuple로 고유 식별.
     * 동일 스레드의 후속 메시지가 기존 세션을 재사용할 수 있도록 한다.
     *
    ...
    public Optional<AiSessionResponse> findBySlackContext(
        ...
        return dsl.select(
                    ID, USER_ID, SESSION_ID, CONTEXT_TYPE, CONTEXT_RESOURCE_ID,
                    TITLE, CREATED_AT, UPDATED_AT,
                    CHANNEL_SOURCE, SLACK_TEAM_ID, SLACK_CHANNEL_ID, SLACK_THREAD_TS)
                .from(AI_SESSION)
                .where(CHANNEL_SOURCE.eq("SLACK")
                ...
                .fetchOptional(r -> new AiSessionResponse(
                        ...
                        r.get(CONTEXT_TYPE), r.get(CONTEXT_RESOURCE_ID),
                        r.get(TITLE), r.get(CREATED_AT), r.get(UPDATED_AT),
                        r.get(CHANNEL_SOURCE), r.get(SLACK_TEAM_ID),
                        r.get(SLACK_CHANNEL_ID), r.get(SLACK_THREAD_TS)));
    }

    /**
     * 새 SLACK 세션을 INSERT하고 생성된 row의 id를 반환한다.
     * session_id에는 ai-agent 서비스가 발급한 외부 세션 ID를 저장한다.
