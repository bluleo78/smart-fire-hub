 ...
 *
 * <p>기존 {@code AiAgentProxyService}는 SSE 스트리밍으로 브라우저 클라이언트에 중계한다.
 * 이 클래스는 단일 응답을 동기 대기(timeout: session 10초, chat 60초)하여 반환한다.
 *
 ...
 * </ul>
 * 실제 ai-agent가 이 엔드포인트/스키마를 지원하지 않는 경우 향후 수정 필요.
 * 통합 테스트에서는 이 클라이언트를 Mockito로 mock하여 검증한다.
 */
@Component
public class AiAgentBatchClient {
     ...
     *
     * <p>stream=false를 body에 포함하여 단일 JSON 응답을 요청한다.
     * ai-agent가 항상 SSE 스트리밍만 지원하는 경우 향후 어댑터 교체 필요.
     *
