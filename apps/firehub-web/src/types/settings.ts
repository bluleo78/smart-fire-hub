 ...
 *
 * - `value`: 키마다 출처가 다르다. AI 동작 설정(`ai.*`)은 워크스페이스 저장값, 없으면 코드 기본값.
 *   SMTP(`smtp.*`)는 워크스페이스 저장값뿐이다(#712) — 저장하지 않은 키는 응답에 아예 없다.
 *   임베딩(`embedding.*`)은 플랫폼 값(읽기 전용). 비밀 키는 `****` 로 마스킹된다.
 * - `overridden`: 지금 보이는 값이 워크스페이스 저장값에서 왔는지(AI 탭의 "기본값" 힌트에 쓴다).
 * - `tenantEditable`: 이 키를 워크스페이스가 바꿀 수 있는지(저장 여부와 무관한 키 고유 성질).
 *
 * `value`/`description`/`updatedAt` 이 null 일 수 있는 이유: 플랫폼 행 없이 워크스페이스 값만
