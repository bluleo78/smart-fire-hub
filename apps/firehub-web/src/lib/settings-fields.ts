...
/**
 * 설정 필드 하나가 화면에서 취하는 네 가지 상태.
 * - `inherited`: 편집 가능 + 플랫폼 기본값을 그대로 쓰는 중
 * - `overridden`: 편집 가능 + 테넌트 재정의가 적용된 중
 * - `locked`: 플랫폼 전용(테넌트 편집 불가)
 * - `no-default`: 편집 가능하지만 플랫폼 기본값 행 자체가 없음(값이 비어 있는 것과 다르다)
 */
export type SettingFieldState = 'inherited' | 'overridden' | 'locked' | 'no-default';
 ...
 * 뜻하므로 `no-default` 다. 이때만 편집 가능 여부를 화이트리스트 상수로 폴백한다.
...
export function resolveSettingFieldState(
  ...
  if (!setting || setting.value === null) return 'no-default';
