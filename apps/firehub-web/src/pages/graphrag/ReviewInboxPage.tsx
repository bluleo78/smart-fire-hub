...
import type {
  PropertyPayload, ReviewItemResponse, ReviewItemType, SynonymPayload,
...
function ReviewRow(props: {
  ...
  return (
    <>
      <TableRow>
        <TableCell>
          <Badge variant="secondary">{isSynonym ? '동의어' : isProperty ? '속성' : row.itemType}</Badge>
        </TableCell>
        ...
        <TableCell className="text-sm text-muted-foreground">
          {row.signalType === 'similarity' && row.signalScore != null ? `유사도 ${row.signalScore.toFixed(3)}` : row.signalType ?? '-'}
          <div>
