 ...
 * 기존 카드의 `entity.type`을 key·식별자로 쓰는 불변식이 유지되고, 타입 리네임은 여전히 범위 밖
 * (node key 파급 — 별도 슬라이스). 삭제 시 그 타입을 참조하던 관계도 함께 정리(cascade)한다.
 *
...
export default function OntologyEditDialog({ schema, open, onOpenChange }: OntologyEditDialogProps) {
  ...
  // 항상 유효·고유해 entity.type을 key/식별자로 쓰는 불변식이 유지된다(리네임은 범위 밖).
  const addEntityType = () => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto" data-testid="ontology-edit-dialog">
        <DialogHeader>
          ...
          <DialogDescription>
            엔티티 타입의 추가·삭제, 각 타입의 설명·명명 규칙·해상도·속성, 엔티티 간 관계를 수정합니다. 타입 이름 변경(리네임)은 지원하지 않습니다.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-6 py-2">
          ...
          {/* 엔티티 타입 추가 — 이름을 진입 전에 검증(non-blank·중복)해 append. 리네임은 미지원 */}
