...
/**
 * AI 채팅 파일 첨부 사이즈 제한 E2E 테스트 (이슈 #41)
 * - ChatInput 컴포넌트의 SIZE_LIMITS 상수 변경 후 정상 동작 검증
 * - 파일 크기가 새 제한값 이내면 첨부 성공, 초과 시 toast 에러 표시 확인
 * - 백엔드 API 불필요 — 파일 크기 검증 로직은 클라이언트 사이드에서 수행됨
 */

test.describe('AI 채팅 파일 첨부 사이즈 제한', () => {
  /**
   * 테스트 1: IMAGE 파일 7MB → 첨부 성공
   * - 변경 전: 5MB 제한 / 변경 후: 10MB 제한
   * - 7MB 이미지를 첨부하면 파일 pill이 렌더링되어야 한다 (변경 전엔 에러)
   */
  test('IMAGE 파일 7MB 첨부가 허용된다 (5MB 제한 → 10MB)', async ({ authenticatedPage: page }) => {
    await page.goto('/');
  ...
  /**
   * 테스트 3: DATA 파일(CSV) 6MB → 첨부 성공
   * - 변경 전: 5MB 제한 / 변경 후: 256MB 제한
   * - 6MB CSV를 첨부하면 파일 pill이 렌더링되어야 한다 (변경 전엔 에러)
   * - 256MB 전체 검증은 Playwright 50MB buffer 한도로 불가 — 5MB 문턱 통과로 대체
   */
  test('DATA 파일(CSV) 6MB 첨부가 허용된다 (5MB 제한 → 256MB)', async ({ authenticatedPage: page }) => {
    await page.goto('/');
  ...
  /**
   * 테스트 4: PDF 파일 15MB → 첨부 성공
   * - 변경 전: 10MB 제한 / 변경 후: 20MB 제한
   * - 15MB PDF를 첨부하면 파일 pill이 렌더링되어야 한다 (변경 전엔 에러)
   */
  test('PDF 파일 15MB 첨부가 허용된다 (10MB 제한 → 20MB)', async ({ authenticatedPage: page }) => {
    await page.goto('/');

    const fileInput = page.locator('input[type="file"]').first();

    // 15MB PDF 파일 (20MB 제한 이내 — 변경 전 10MB 제한 초과, 변경 후 허용)
    await fileInput.setInputFiles({
      name: 'document.pdf',
