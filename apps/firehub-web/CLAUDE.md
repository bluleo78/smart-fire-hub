...
- Lines (E2E 기준, MCR V8): **80.15%** (24,883/31,047) — 목표 80% 달성 ✅
- Lines (Istanbul 기준): **81.94%** — 목표 달성 ✅
...
pre-commit 단계에서 핵심 회귀를 빠르게 검증하기 위해 `@smoke` 태그를 사용한다. 분류는 의미 기반 — 정규식·키워드가 아니라 다음 기준으로 사람/AI가 판단한다.

### Smoke 인 것 (포함)
1. **Happy path** — 정상 입력 + 정상 API 응답에서 의도된 기본 결과가 나오는지 검증
2. **핵심 사용자 경로** — 사용자가 빈번히 거치는 흐름 (페이지 진입 + 핵심 데이터 표시, 도메인 대표 CRUD 1개, 핵심 위젯 진입)
3. **깨지면 도메인 전체가 사실상 불능** — 이 test 가 실패하면 그 도메인의 다른 동작이 의미 없을 정도

### Smoke 아닌 것 (제외)
1. **에지 케이스** — 잘못된 입력, 4xx/5xx 응답, 빈 상태, 권한 거부, 토큰 만료
2. **회귀 검증 전용** — 특정 이슈(#xx)의 재현 방지를 위한 test
3. **UI 미세 동작** — 호버, 툴팁, 접근성 속성, 정렬, 키보드 단축키, 스크롤 디테일
4. **통합 시나리오** — 여러 페이지/세션을 거치는 복합 흐름 (대부분 `e2e/flows/*`)

### 판단 보조 — test 하나를 두고 자문
- 이게 깨지면 사용자가 페이지를 못 쓰는가?
- 매일 거치는 흐름인가?
- "정상" 또는 "기본" 동작을 검증하는가?

위 셋 중 **둘 이상 yes** → smoke 후보. 아니면 제외.

### 부착 방법
```ts
test('데이터셋 목록이 렌더링된다', { tag: '@smoke' }, async ({ authenticatedPage: page }) => {
  // ...
});
```

### 분포 기대치
- 도메인당 평균 1~3개 smoke (페이지 진입, 핵심 CRUD, 위젯)
- 한 spec 안에 여러 test 가 있어도 smoke 는 보통 1~2개 (가장 본질적인 것만)
- 총 ~80~120개 (전체 ~852 의 10~14%)

### 새 spec 작성 시
- 기본 동작 검증 test 를 가장 먼저 작성하고 `{ tag: '@smoke' }` 부착
- 이슈/에지는 그 다음 또는 별도 describe 블록으로 분리

### 게이트 동작 (#257)
- `src/pages/<domain>/` 단독 변경: 해당 도메인 e2e + `@smoke`
- 공유 영역(components/api/lib/hooks/types) 또는 매핑 모호: 전체 E2E
- 비코드 단독: ~5s 통과
- push 시점(`pre-push`): 전체 E2E + gradle 풀 회귀 (안전망)
