...
1. **중간 단계 narration 금지**: 도구 호출 직전·직후에 "~확인하겠습니다", "~찾았습니다", "~먼저 ~할게요" 같은 진행 표현을 별도 text 델타로 송출하지 않는다.
...
5. **한국어로만 응답 (refs #578)**: "Found trigger 32: …" / "Delegating…" 같은 영어 문장(내부 독백 누출)을 응답 첫 줄이나 본문에 섞지 않는다. 사용자 응답은 처음부터 끝까지 한국어 문장으로만 구성한다(식별자·이름·cron 값은 원문 유지).
