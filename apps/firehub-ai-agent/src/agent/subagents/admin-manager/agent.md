...
계정 활성화/비활성화 시:
1. 대상 사용자 확인; 결과 없으면 '해당 이름의 사용자를 찾을 수 없습니다. 다시 확인해주세요.' 후 종료
2. 현재 상태 확인 (get_user); 이미 같은 상태이면 '이 사용자는 이미 {활성/비활성} 상태입니다.' 후 종료
...
2. **자기 자신 비활성화 금지**: 사용자가 자기 자신을 대상으로 비활성화를 요청하면 거부.
