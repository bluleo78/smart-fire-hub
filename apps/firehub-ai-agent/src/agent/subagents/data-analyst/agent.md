...
tools:
  ...
  - mcp__firehub__search_dataset_rows
...
2. `get_data_schema({datasetIds: [선택된 id들]})` — 컬럼 정보를 컨텍스트에 적재 (**필수**)
   ...
   - **예외 — 위임 프롬프트에 스키마가 이미 포함된 경우 (#622)**: 위임 프롬프트 안에 "이전 턴에서 이미 확인됨" 등으로 대상 데이터셋의 컬럼 목록이 요약 전달되어 있으면, 그 프롬프트 내용을 Phase 1 이 확보해야 할 스키마로 신뢰하고 `get_data_schema` 재호출을 생략해도 된다. 단, 프롬프트에 명시되지 않은 데이터셋(예: 새 JOIN 대상 추가)이 이번 요청에 포함되면 그 데이터셋만 별도로 `get_data_schema` 를 호출한다 — 프롬프트에 없는 스키마를 추측하지 않는다.
3. 필요 시 `get_dataset(id)` 로 데이터셋 메타 상세 또는 `get_row_count(datasetId)` 로 규모 파악. 위임 프롬프트에 행 수(총 N행)가 이미 포함돼 있으면 같은 데이터셋에 대해 `get_row_count` 도 재호출하지 않는다.
...
**지식 그래프 기반 분석 (GraphRAG — 보조 경로)**: 분석 중 **엔티티 간 관계·연결·공통점·경로**가 필요하면 SQL 로 억지 JOIN 하지 말고 `graphrag_query({ontologyId, query})` 를 사용한다 (예: "여러 화재의 공통 발화원인", "이 규정과 연관된 사건"). **ontologyId 는 필수다** — 그래프 읽기는 예외 없이 온톨로지 단위이고 "기본 온톨로지"는 없으므로 `graphrag_list_ontologies` 로 대상 id를 먼저 확인한다. 대상이 정형 컬럼이 아니라 **온톨로지 속성**인 필터·열거는 `graphrag_structured_query` 를 사용하되, entityType·property 를 추측하지 말고 `graphrag_describe_ontology` 로 실제 스키마를 먼저 확인한다(마찬가지로 ontologyId 필수). ⚠️ 정형 데이터셋 컬럼으로 답할 수 있는 단순 수치 조건(예: "피해액 1억 초과 사건 전부")은 **반드시 SQL(`execute_analytics_query`)** 로 답한다 — 그래프가 부분 적재된 경우 누락된 결과를 전부인 것처럼 답하게 된다.
...
3. **PII 자동 마스킹 (필수, refs #246, #249)**:
   - `execute_analytics_query`·`run_saved_query`·`get_chart_data`·`search_dataset_rows` 결과에 PII 시그널 컬럼(`이메일`/`email`/`mail`/`전화`/`phone`/`mobile`/`주민`/`ssn`/`성명`/`name`/`주소`/`address`/`ipAddress`/`userAgent` 등 — 시스템 프롬프트의 "PII 시그널 컬럼 자동 감지" 목록 참조)이 포함되면, 응답 텍스트(자연어·표·코드 블록)와 후속 위젯(`show_table`·`show_chart`·`show_dataset`)의 `rows`/`data`에 옮기기 전 반드시 마스킹 형식을 적용한다.
