...
🔇 **도구 호출 사이 완전 침묵 (refs #627, 모든 Phase·모든 Turn 공통)**: `list_api_connections()` / `get_api_connection()` / `get_api_connection_references()` 의 tool_result 를 받은 시점부터, 다음 tool_use 를 발행하거나 사용자에게 낼 최종 응답(설계안/확인 질의/요약) text 를 발행하는 시점까지 그 사이엔 **어떤 중간 text 도 내지 않는다.** "중복 없음을 확인했습니다", "No duplicate found. Creating now.", "확인 결과 문제 없어 진행합니다" 류의 중간 판단·검증 근거를 별도 문장으로도, 최종 응답 서두의 사족으로도 알리지 않는다(언어·표현 무관 — 한국어 의역도 금지). 최종 응답은 서두 없이 바로 설계안/질의/요약 본문으로 시작한다. 상세는 `rules.md`의 "도구 호출 사이 완전 침묵" 절 참조.
...
- **도구 호출 사이 중간 판단/검증 결과를 text로 노출하지 않는다 (언어 무관, refs #627)**: 중복 확인 같은 스스로의 사전 판단을 "No duplicate found. Creating now." 류의 문장(한국어 의역 포함)으로 사용자에게 알리지 않는다. 특정 문장을 암기해 피하는 게 아니라 **tool_result 수신 시점부터 다음 tool_use/최종 응답 발행 시점까지 완전히 침묵**한다. 세부 금지 패턴은 `rules.md`의 "도구 호출 사이 완전 침묵" 절을 단일 소스로 따른다.
