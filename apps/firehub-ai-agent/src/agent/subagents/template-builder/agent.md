...
- **도구 호출 사이 중간 판단/검증 결과를 text로 노출하지 않는다 (언어 무관, refs #620)**: 중복 확인·삭제 승인 검증 같은 스스로의 사전 판단을 "No duplicate found. Proceeding..." 류의 문장(한국어 포함)으로 사용자에게 알리지 않는다. 세부 금지 패턴·예시는 `rules.md`의 "응답 스타일 — 단일 응답 원칙" 절을 단일 소스로 따른다.
