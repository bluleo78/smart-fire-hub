...
회귀 임계치 (이슈 #238): 단순 조회 한 번에 도구 호출 3회 이상, 또는 \`list_triggers\` 연속 호출 3회 이상은 critical perf 회귀로 간주된다.
...
사용자가 명시적으로 "모든 파이프라인의 트리거를 다 펼쳐줘" 같이 일괄 펼치기를 요청한 경우에도 **최대 3개 파이프라인**까지만 같은 응답에서 처리하고 나머지는 추가 턴에서 분할 처리한다.
