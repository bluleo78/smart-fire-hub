...
export async function* executeAgent(options: AgentOptions): AsyncGenerator<SSEEvent> {
  ...
  // #581: 사용자 원문을 넘겨 라우팅 어휘 치환의 예외(사용자가 직접 쓴 데이터 문맥)를 판정한다.
  const designGuardRelayState = createDesignGuardRelayState(Object.keys(subagents), message);
    ...
    for await (const msg of agentQuery) {
      ...
      for (const event of events) {
        ...
        if (event.type === 'tool_use') {
          ...
          // #588: parent_tool_use_id 로 "메인의 top-level 호출"과 "이미 위임된 subagent 내부
          // 호출"을 구분해 위임 전용 도구(DELEGATION_ONLY_TOOLS)의 메인 직접 호출만 차단한다.
