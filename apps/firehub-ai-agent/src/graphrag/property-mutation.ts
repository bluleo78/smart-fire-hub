...
/**
 * ontologyId 로 스코프된 entityKey 노드의 propertyName 속성값을 정정값으로 설정한다.
 ...
 *
 * ontologyId 술어가 필요한 이유는 verified-ontology-id.ts 참고(크로스테넌트 논거의 정본).
 * 여기서 추가로 지킬 것 하나: 스코프 밖 키는 "없는 키"와 **똑같은** GraphTargetMissingError 가
 * 되어야 한다 — 사유 문구가 갈리면 그 자체가 "그 키가 남의 그래프에 있다"를 알려주는 읽기 오라클이 된다.
 */
export async function setEntityProperty(
  ontologyId: VerifiedOntologyId, entityKey: string, propertyName: string,
  dataType: 'text' | 'number' | 'date', value: string,
): Promise<void> {
  ...
  try {
    ...
    const result = await session.run(
      'MATCH (n:Entity {key: $key}) WHERE n.ontologyId = $ontologyId '
        + 'SET n += $props RETURN count(n) AS updated',
      { key: entityKey, ontologyId: ontologyIdParam(ontologyId), props },
    );
