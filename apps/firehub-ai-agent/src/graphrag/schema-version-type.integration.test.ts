...
describe('schemaVersion 저장 타입 (실 Neo4j, #308)', () => {
  ...
  // FLOAT로 적재된 과거 데이터가 섞여 있어도 그래프 읽기가 죽지 않아야 한다(읽기측 방어).
  // 읽기가 온톨로지 스코프로 좁혀지므로 이 레거시 노드에도 ontologyId를 심는다 — 스코프 밖이면
  // 애초에 반환되지 않아 FLOAT 방어 경로를 타지 못하고, 테스트가 조용히 공허해진다.
  // ontologyId는 INTEGER로, schemaVersion은 의도적으로 FLOAT로 둔다(검증 대상이 후자다).
  it('FLOAT로 저장된 레거시 노드가 있어도 readWholeGraph가 throw하지 않는다', async () => {
    const s = getSession();
    try {
      await s.run(
        'MERGE (n:Entity {key:$key}) SET n.name=$name, n.type=$type, n.schemaVersion=1.0, n.ontologyId=$ontologyId',
        { key: kA, name: 'ZZTEST_308사건', type: 'Incident', ontologyId: neo4j.int(9) },
      );
    } finally { await s.close(); }
