...
vi.mock('../../graphrag/ontology-source.js', () => ({
  loadOntology: vi.fn(),
  // ingest 는 이제 바인딩 기반으로 온톨로지를 해소한다. 이 테스트는 적재 이력 기록만 검증하므로
  // 해소 결과는 최소 형태로 고정한다.
...
describe('graphrag_ingest 도구 — 적재 이력 best-effort 기록', () => {
  ...
  it('추출 실패가 0건이면 status=SUCCESS 로 recordGraphIngest 를 호출한다', async () => {
    vi.mocked(resolveDatasetOntology).mockResolvedValue({ ontology, ontologyId: 42, source: 'binding' } as never);
    ...
    // 바인딩 해소 경로가 실제로 타는지(기본/번들 폴백으로 새지 않는지) 못박는다.
    expect(payload.ontologyId).toBe(42);
    expect(payload.ontologySource).toBe('binding');
